        ...
        changeColor: (newcolor) => {
            if (products.inventory.colorOptions?.includes(newcolor)) {
                ...
                if (newcolor == "red") {
                    ...
                    return chalk.redBright(`you select ${products.name} of color "${products.color}" and its price is Rs${products.price}`);
                }
                else if (newcolor == "blue") {
                    products.price *= 0.95;
                    console.log(chalk.green.bold.underline(`Enjoy 5% off on ${newcolor}  ${products.name}!`));
                    return chalk.blueBright.bold.italic(`you select ${products.name} of color "${products.color}" and its price is Rs${products.price}`);
                }
                else {
                    return chalk.magentaBright.bold.italic(`you select ${products.name} of color "${products.color}" and its price is Rs${products.price}`);
                }
            }
            else {
                return chalk.cyanBright.strikethrough(`Color "${newcolor}" is not available.`);
            }
        },
    },
};
console.log(chalk.hex("#f77e1a").bold.bgHex("#fff2cc")("\t\t\t\t\t\t T-shirt Store"));
...
console.log(chalk.hex("93FF33")(`Products available colors: ${products.inventory.colorOptions}`));
